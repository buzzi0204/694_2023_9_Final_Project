...
class Cache:
    ...
    def __init__(self, checkpoint_file=None, checkpoint_interval=None):
        ...
        self.checkpoint_interval = checkpoint_interval  # Interval for saving cache state
        if checkpoint_file:
    ...
    def get(self, key):
        ...
        if key in self.cache:
            return self.cache[key]  # Return the value associated with the key if it's in the cache
        else:
    ...
    def set(self, key, value):
        ...
        if len(self.cache) >= self.max_size:
            ...
            oldest_key = min(self.key_times, key=lambda x: x[1])[0]  # Find the oldest key using key_times
            self.key_times = [x for x in self.key_times if x[0] != oldest_key]  # Remove the oldest key from key_times
        ...
        self.key_times.append((key, time.time()))  # Add the current time to key_times for the new key
    ...
    def save_checkpoint(self):
        ...
        with open(self.checkpoint_file, 'wb') as file:
            pickle.dump(self.cache, file)  # Serialize the cache dictionary and save it to the checkpoint file
    ...
    def load_checkpoint(self):
        ...
        try:
            with open(self.checkpoint_file, 'rb') as file:
                self.cache = pickle.load(file)  # Deserialize the cache dictionary from the checkpoint file
                self.key_times = [(k, time.time()) for k in self.cache.keys()]  # Reset the key_times list
